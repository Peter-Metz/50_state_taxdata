...
fvec2 <- function(betavec, wh, xmat, targets){
  g <- grad(fvec, x=betavec, wh=wh, xmat=xmat, targets=targets)
...
betavec0 <- as.vector(beta0)
fvec(betavec0, hweights, sxmat, stargets)
fvec2(betavec0, hweights, sxmat, stargets)

res1 <- nlm(fvec, betavec0, wh=hweights, xmat=sxmat, targets=stargets, stepmax=1e2)
res1


res2 <- nlm(fvec2, betavec0, wh=hweights, xmat=sxmat, targets=stargets)
res2

res <- res2
bres <- matrix(res$estimate, nrow=nrow(stargets), byrow=FALSE)
dres <- get_delta(hweights, bres, sxmat)
wres <- get_weights(bres, dres, sxmat)
wres
colSums(wres)
rowSums(wres)
hweights
hweights - rowSums(wres) %>% round(1)
tres <- t(wres) %*% sxmat
d <- stargets - tres
d %>% round(2)
sum(d^2)
...
g <- grad(fvec, x=betavec0, wh=hweights, xmat=xmat, targets=targets)
